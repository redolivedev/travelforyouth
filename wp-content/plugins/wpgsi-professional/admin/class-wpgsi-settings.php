...
class Wpgsi_Settings {
	 ...
	 */
	public function __construct( $plugin_name, $version, $eventsObj, $googleSheet ) {
	...
	public function wpgsi_settings_view() {
		...
		if ( ! @fsockopen('www.google.com', 80 ) ){
			$this->events->wpgsi_log( get_class($this),__METHOD__,"914","Error: no internet connection ! from settings view page !");
			...
			if ( isset( $token['expires_in'] ) && time() > $token['expires_in'] ) {
				...
				if ( $credential ) {
					...
					if ( $new_token[0] ) {
						...
						update_option( 'wpgsi_google_token', $token );
					} else {
						$this->events->wpgsi_log( get_class($this), __METHOD__,"701", "Error : on creating token. " . json_encode( $new_token ) );
					}
				} else {
					$this->events->wpgsi_log( get_class($this), __METHOD__,"701", "Error: get_option('wpgsi_google_credential'); is FALSE or Empty" );
				}
			}
		} else {
			$this->events->wpgsi_log( get_class($this), __METHOD__,"701", "Error: get_option('wpgsi_google_token'); is FALSE or Empty" );
		}
		...
		if ( $action == 'log' ) {
			...
			echo"<div class='wrap'>";
				echo"<h1 class='wp-heading-inline'>  Log Page ";
					...
					echo"<code>V". $this->version ."</code> &#32;&#32; <code>". $this->events->wpgsi_integrations()[2] ." Active Integration</code> &#32;&#32; <code>". $this->events->wpgsi_integrations()[3]." Hold Integration</code>";
				echo"</h1>";
	...
	public function wpgsi_google_settings() {
		...
		if ( $credential  &&  wp_verify_nonce( $_POST['nonce'], 'wpgsi-google-nonce' ) ) {
			...
			if ( isset( $credential['private_key'], $credential['client_email'] ) ) {
				...
				$token = $this->googleSheet->wpgsi_token( $credential );
				# Check and Balance 
				if ( $token[0] ) {
					...
					#
					$this->events->wpgsi_log( get_class($this), __METHOD__, "702", "Error: false credential ! Google said so ;-D ." . json_encode( $token ) );
					wp_redirect( 'admin.php?page=wpgsi-settings&msg=false' );
				}
			} else {
				# if credential vitals are empty or Missing 
				$this->events->wpgsi_log( get_class($this), __METHOD__, "703", "Error:  private_key or client_email is Not set ! ");
				wp_redirect( 'admin.php?page=wpgsi-settings&msg=false' );
			}
		} else {
			# if credential vitals are empty or Missing 
			$this->events->wpgsi_log( get_class($this), __METHOD__, "705", "Error: Something missing when you copied the key ! that not JSON.");
			wp_redirect( 'admin.php?page=wpgsi-settings&msg=false' );
		}
