...
class THWMSC_License_Manager {
	...
	private function output_license_form($status, $license_data){
		...
		if($status === self::STATUS_ACTIVATED){
			...
			$btn_label  = __('Activate', 'woocommerce-multistep-checkout');
			$btn_action = 'activate';

			$license_form_title_note = __('Enter your License Key and hit activate button.', 'woocommerce-multistep-checkout');
		...
		?>
		<h1><?php _e('Software License Key', 'woocommerce-multistep-checkout') ?></h1>
